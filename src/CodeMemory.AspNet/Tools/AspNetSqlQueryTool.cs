...
Execute SELECT-only SQL queries against the AspNet relational storage backend.
        ...
        int maxResults = 100)
    {
        ...
        try
        {
            ...
            if (hybridStorage is null)
                return Fail("AspNet sql_query requires HybridStorageService. The current storage provider does not expose relational SQL storage.", sw);
        ...
        {
            logger.LogError(ex, "AspNet SQL query execution failed: {Query}", query);
