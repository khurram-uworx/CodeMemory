...
public sealed class StorageServiceRouter : IStorageService
{
    ...
    public StorageServiceRouter(IStorageServiceRegistry registry, IRepoContextAccessor repoContext)
    {
        this.registry = registry;
        this.repoContext = repoContext;
    }

    IStorageService GetStorage() => registry.GetStorage(repoContext.CurrentRepoName);

    public Task InitializeAsync(CancellationToken ct = default) => GetStorage().InitializeAsync(ct);
    ...
    public Task ClearAllAsync(CancellationToken ct = default) => GetStorage().ClearAllAsync(ct);
}