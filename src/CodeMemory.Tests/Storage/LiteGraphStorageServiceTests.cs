...
public sealed class LiteGraphStorageServiceTests
{
    static (string repoRoot, string dbPath) GetTempDbPath()
    {
        var dir = Path.Combine(Path.GetTempPath(), "CodeMemoryTests", Guid.NewGuid().ToString());
        Directory.CreateDirectory(dir);

        return (dir, Path.Combine(dir, "litegraph.db"));
    }

    static LiteGraphStorageService CreateEphemeralStorage()
    {
        var repoRoot = Path.Combine(Path.GetTempPath(), "CodeMemoryTests", Guid.NewGuid().ToString());
        Directory.CreateDirectory(repoRoot);

        return new LiteGraphStorageService(
            repoRoot,
            new LiteGraphStorageOptions { Ephemeral = true },
            NullLogger<LiteGraphStorageService>.Instance);
    }

    static LiteGraphStorageService CreateStorage(string repoRoot, LiteGraphStorageOptions options)
        => new LiteGraphStorageService(repoRoot, options, NullLogger<LiteGraphStorageService>.Instance);

    [Test]
    public async Task InitializeAsync_CompletesSuccessfully()
    {
        var storage = CreateEphemeralStorage();
        await storage.InitializeAsync();
        Assert.That(storage.RepoRoot, Is.Not.Null);
    }
    ...
    public async Task StoreRelationshipsAsync_And_GetRelationshipAsync_RoundTrip()
    {
        ...
        // LiteGraph edges require source and target nodes to exist
        await storage.StoreSymbolsAsync([
            new SymbolRecord { Id = "ClassA", Name = "ClassA", Kind = "Class",
                FilePath = "/src/A.cs", LineStart = 1, LineEnd = 10, FullName = "ClassA" },
            new SymbolRecord { Id = "ClassB", Name = "ClassB", Kind = "Class",
                FilePath = "/src/B.cs", LineStart = 1, LineEnd = 10, FullName = "ClassB" }
        ]);
    ...
    [Test]
    public async Task EphemeralMode_DataLostAfterDispose()
    {
        var storage = CreateEphemeralStorage();
        await storage.InitializeAsync();
        ...
        await storage.StoreSymbolsAsync([
            new SymbolRecord
            {
                Id = "ephemeral", Name = "Ephemeral", Kind = "Class",
                FilePath = "/src/Test.cs", LineStart = 1, LineEnd = 10,
                FullName = "Ephemeral"
            }
        ]);

        var retrieved = await storage.GetSymbolAsync("ephemeral");
        Assert.That(retrieved, Is.Not.Null);

        storage.Dispose();

        var storage2 = CreateEphemeralStorage();
        await storage2.InitializeAsync();

        var afterDispose = await storage2.GetSymbolAsync("ephemeral");
        Assert.That(afterDispose, Is.Null);
    }

    [Test]
    public async Task NonEphemeralMode_DataSurvivesAcrossInstances()
    {
        (var repoRoot, var dbPath) = GetTempDbPath();

        var options = new LiteGraphStorageOptions
        {
            Ephemeral = false,
            Filename = dbPath
        };

        var storage1 = CreateStorage(repoRoot, options);
        ...
        // Need source/target symbol nodes for edge validation
        await storage1.StoreSymbolsAsync([
            new SymbolRecord { Id = "A", Name = "A", Kind = "Class",
                FilePath = "/src/A.cs", LineStart = 1, LineEnd = 10, FullName = "A" },
            new SymbolRecord { Id = "B", Name = "B", Kind = "Class",
        ...
        var storage2 = CreateStorage(repoRoot, options);
        await storage2.InitializeAsync();
    ...
    public async Task GetRelationshipsBySourceAsync_ReturnsMatching()
    {
        ...
        // Source/target nodes must exist for LiteGraph edge validation
        await storage.StoreSymbolsAsync([
            new SymbolRecord { Id = "A", Name = "A", Kind = "Class",
                FilePath = "/src/A.cs", LineStart = 1, LineEnd = 10, FullName = "A" },
            new SymbolRecord { Id = "B", Name = "B", Kind = "Class",
    ...
    public async Task SearchChunksAsync_ReturnsResultsOrderedByScore()
    {
        ...
        far[0] = 0.5f;
        far[1] = 0.8660254f; // 60° from query direction → cosine sim ≈ 0.5
    ...
    public async Task ClearAllAsync_RemovesAllData()
    {
        ...
        await storage.StoreChunksAsync([
            new ChunkRecord { Id = "c1", SymbolId = "s1", FilePath = "/src/A.cs",
                Content = "test", Language = "CSharp", LineStart = 1, LineEnd = 10,
                Embedding = new float[TestConstants.EmbeddingDimension].AsMemory() }
        ]);

        await storage.ClearAllAsync();

        var symbol = await storage.GetSymbolAsync("s1");
        var chunk = await storage.GetChunkAsync("c1");
    ...
    [Test]
    public async Task ClearAllAsync_CanReuseStore()
    {
        var storage = CreateEphemeralStorage();
        await storage.InitializeAsync();
        ...
        var retrieved = await storage.GetSymbolAsync("s2");
        Assert.That(retrieved, Is.Not.Null);
        Assert.That(retrieved!.Name, Is.EqualTo("S2"));
    }
